...
# docs/skepticism/ dir grows unbounded and every future run pays for it.
set -uo pipefail
...
seed_run() { # <slug> <feature> <tier>
  mkdir -p "docs/skepticism/$1"
  printf '# Skepticism run: %s\n\n- feature: %s\n- tier: %s\n- phase: done\n' "$1" "$2" "$3" \
    > "docs/skepticism/$1/state.md"
}

seed_run login "add login" standard
out="$(bash "$CONSOL" login 2>&1)"; rc=$?
assert_eq            "consolidate exits 0"              0 "$rc"
assert_file          "index is created"                "docs/skepticism/INDEX.md"
assert_file_contains "index names the run"             "docs/skepticism/INDEX.md" "login"
assert_file_contains "index carries the feature"       "docs/skepticism/INDEX.md" "add login"
assert_file_contains "index carries the tier"          "docs/skepticism/INDEX.md" "standard"
assert_file          "artifacts moved into archive"    "docs/skepticism/archive/login/state.md"
assert_absent        "live run dir is gone"            "docs/skepticism/login"
...
assert_file_contains "second run appended to index"    "docs/skepticism/INDEX.md" "logout"
assert_file_contains "first run still indexed"         "docs/skepticism/INDEX.md" "login"
headers=$(grep -c 'completed runs index' "docs/skepticism/INDEX.md")
